...
import { listSandboxes, type SandboxView } from '../api/admin';
import type { MeResponse } from '../auth';

const STATUSES = ['', 'active', 'released', 'evicted'];
const TYPES = ['', 'e2b', 'cube', 'docker'];
const TABLE_GRID = '120px 110px 150px 180px 180px 160px 160px';
  ...
  return (
    <div style={{ padding: '34px 40px', maxWidth: 1280 }}>
      ...
      <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: 10, overflowX: 'auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: TABLE_GRID, minWidth: 1060, gap: 0, background: '#f8fafc', borderBottom: '1px solid #e2e8f0', color: '#475569', fontSize: '0.76rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
          {['Status', 'Type', 'User', 'Agent', 'Session', 'Last used', 'Expires'].map(h => (
        ...
        {!loading && rows.map(row => (
          <div
            key={row.id}
            style={{ display: 'grid', gridTemplateColumns: TABLE_GRID, minWidth: 1060, borderBottom: '1px solid #f1f5f9', color: '#334155', fontSize: '0.84rem' }}
          >
            <div style={bodyCell}>
            ...
            <div style={bodyCell}>{formatDate(row.expiresAt)}</div>
          </div>
        ))}
      </div>
    </div>

...
- 已完成 Run/Task 的 Token、成本、模型调用次数和时间预算，模型实际用量通过行锁原子扣减；
- 已完成预算预检、执行流 deadline 取消、过期任务后台扫描，以及超限后的 Run/Task/Attempt/Agent 一致终止、事件和 Outbox 审计；
- 已完成父子 Agent 不可扩权的权限快照继承、规范化哈希校验和继承事件审计；
- 已通过 H2 与 PostgreSQL 17.5 的迁移、并发扣减、JSONB 快照、父子继承和超限终止验证。

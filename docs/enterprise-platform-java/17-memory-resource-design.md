...
1. 加管理面查询：按 org/user/session 查看记忆事件和同步状态。
2. 把 `MEMORY.md` consolidation 的摘要结果也写入 PG，形成可审计的摘要版本链。
3. 端到端验证 E2B/CubeSandbox 同类运行时：网页登录、发起任务、沙箱内生成文件、release 前投影、网页下载文件、MinIO snapshot 恢复。CubeSandbox 可后续复跑，不阻塞当前闭环。
4. 补 workspace projection reconciliation：在明确路径作用域后同步删除/移动，避免误删 session mirror 等非沙箱物化文件。

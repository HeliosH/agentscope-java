...
- 新增 sandbox 后端释放状态持久化：`sandboxes` 表记录 `backend_release_status`、`backend_release_attempts`、`backend_released_at`、`backend_release_error`，让 E2B/OpenSandbox/Cube 这类 provider 资源释放可查询、可告警、可重试。
...
- 新增 GitHub 手工 gate `enterprise-runtime-gate.yml`。E2B gate 是当前必须通过的沙箱闭环验证；OpenSandbox gate 保留为可切换后端验证；CubeSandbox 不在本轮强制验证范围内。

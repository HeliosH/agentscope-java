...
4. 端到端验证 CubeSandbox：网页登录、发起任务、沙箱内生成文件、release 前投影、网页下载文件、MinIO snapshot 恢复。
5. 补 workspace projection reconciliation：在明确路径作用域后同步删除/移动，避免误删 session mirror 等非沙箱物化文件。
6. 压测资源释放：沙箱 TTL、并发执行保护、MinIO snapshot 恢复、Redis 断连降级。
...
- `cube-e2e-smoke.sh` 已参数化为通用 sandbox smoke，可用同一套 app 通过 `SAAS_SANDBOX_TYPE=cube|e2b|docker` 验证命令执行和 release 后文件下载。2026-06-25 使用当前 E2B key 验证时，SaaS 组装到 `SandboxBackedFilesystem` 且 remote projection 启用，但 E2B 创建沙箱返回 401 `Invalid API key`，需要可用 key 后复跑。

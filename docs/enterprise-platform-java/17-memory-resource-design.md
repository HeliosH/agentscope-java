...
1. 把 `MEMORY.md` consolidation 的摘要结果也写入 PG，形成可审计的摘要版本链。
2. 端到端验证 E2B/CubeSandbox 同类运行时：网页登录、发起任务、沙箱内生成文件、release 前投影、网页下载文件、MinIO snapshot 恢复。CubeSandbox 可后续复跑，不阻塞当前闭环。
3. 补 workspace projection reconciliation：在明确路径作用域后同步删除/移动，避免误删 session mirror 等非沙箱物化文件。
4. 压测资源释放：沙箱 TTL、并发执行保护、MinIO snapshot 恢复、Redis 断连降级。

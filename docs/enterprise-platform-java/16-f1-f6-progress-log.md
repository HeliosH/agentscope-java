...
## F3-S2 e2e 验证(2026-06-23,cube on + Redis on,call 外 skill 管理全绿)

用户提供 dev Redis(192.168.6.15:6379/2,password `redis@2025`,URI `redis://:redis%402025@192.168.6.15:6379/2` —— `@` 需 URL 编码 `%40`)。gateway profile + cube on + Redis on + 真 LLM(glm-5.1)。

**验证通过(call 外 skill 完整管理闭环)**:
1. call 外 `GET /workspace/memory` → **200**(F3-S2 前 500)
2. call 外 `PUT /skills/workspace/greeter` → **200**(F3-S2 前 500)
3. call 外 `GET /skills/workspace` → **列出 greeter**(name/description/sizeBytes)
4. call 外 `GET /skills/workspace/greeter` → **返回完整 SKILL.md 内容**

启动日志确认接线:`Sandbox remote projection enabled (out-of-call file IO delegates to BaseStore: RedisStore)` + `[harness] Sandbox filesystem remote projection enabled`。

**e2e 发现并修复的 2 个 bug**:
- **`SandboxBackedFilesystem.uploadFiles` call 外仍抛**:原实现只覆盖 read/write/edit/exists/ls 降级,uploadFiles 开头 `requireSandbox()` 在 call 外直接抛 → call 外 PUT skill 经 `BakedContextFilesystem.uploadFiles` → `SandboxBackedFilesystem.uploadFiles` → 500。修复:uploadFiles 开头加 `if (sandbox==null && remoteFallback!=null) return remoteFallback.uploadFiles(...)` 降级。单测 +1(outOfCallUploadDelegatesToFallback)。
- **`AgentSkillsController.writeUtf8` 路径与 ls/read 不一致**:writeUtf8 主动去前导 `/`(`absPath.substring(1)` → key `skills/greeter/SKILL.md`),但 `ls`/`read`/`exists` 用 `normalizePath` 加前导 `/`(查 `/skills/greeter/SKILL.md`)。沙箱内时沙箱文件系统容忍(tar/sed 不在乎前导 `/`),所以沙箱 on 一直工作;但 Redis KV key 精确匹配,不容忍 → call 外 PUT skill 写入但 GET 列表读不到(404)。修复:writeUtf8 改为加前导 `/`(`absPath.startsWith("/") ? absPath : "/" + absPath`),与 ls/read key 一致。
- **尝试过框架级 RemoteFilesystem normalize(write/uploadFiles/edit 改用 normalizePath key)但回退**:破坏 `MemoryConsolidatorFilesystemTest`(测试辅助方法直接 `store.put(ns, "memory/...")` 无前缀,与 read normalize 不匹配)。既有测试依赖"store.put 原始 key"默契,框架 normalize 改动风险高。改用 SaaS 侧 writeUtf8 局部修(只影响 SaaS controller,不破坏框架既有契约)。

**v1 边界(e2e 确认)**:`execute` 工具(直接 shell 写)和 `write_file` 工具(`Files.write` 本地磁盘)不经 `SandboxBackedFilesystem.uploadFiles`,不投影 Redis。call 内用 execute/write_file 写的文件,call 外读 Redis 投影读不到(404)。双写只覆盖经 `uploadFiles` 的写(call 外 PUT skill 经 writeUtf8→uploadFiles,已验证工作)。call 内 LLM 用 execute 写的文件要等 stop snapshot 才进 PG/MinIO 快照(权威副本),call 外读快照需解 tar(未实现端点)。这是 v1 限制,call 内 execute 写文件的 call 外即时读需工具层改造(非 F3-S2 范围)。

**测试 gate**:harness 517/517(含 MemoryConsolidatorFilesystemTest 回归)+ saas-app 31/31。

**关键 gotcha**:
- **Redis URI 密码含 `@` 需 URL 编码**:`redis@2025` → `redis%402025`。`new UnifiedJedis(uri)` 用 RedisURI 解析,未编码的 `@` 会被当 user/host 分隔符。
- **框架 normalize vs SaaS 局部修**:框架 RemoteFilesystem 的 write/uploadFiles 用原始 key、read/exists/ls 用 normalizePath 是既有不一致,既有测试(MemoryConsolidatorFilesystemTest 直接 store.put)依赖原始 key 契约。F3-S2 暴露此不一致到 SaaS controller 路径,修在 SaaS 侧(writeUtf8 加前导 `/`)不破坏框架契约。
- **`execute`/`write_file` 工具不经 uploadFiles**:execute 直接 shell(沙箱内),write_file 用 `java.nio.file.Files.write`(本地磁盘,经 WorkspaceManager 本地路径)。两者都不触发 SandboxBackedFilesystem 双写。call 外读这些工具写的文件需走快照(权威副本)或工具层改造。

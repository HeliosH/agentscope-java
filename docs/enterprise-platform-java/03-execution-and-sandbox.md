...
SAAS_SANDBOX_OPENSANDBOX_API_BASE_URL=http://opensandbox.internal:8080
...
## 7. 文件持久化（文件目录 + 对象存储 + 工作区投影）⭐

> Java 方案不用 JuiceFS/FUSE。工作区仍用框架 `AbstractFilesystem` 作为运行时读写面；企业级持久化由 PG 文件目录和对象存储承载，Redis/沙箱目录只作为可重建投影。
...
read_file("MEMORY.md")
  → SandboxBackedFilesystem (proxy)
      ...
      └─ 沙箱不活跃：CompositeFilesystem
           ├─ RemoteFilesystem → BaseStore.get(namespace, key)  ← 运行时投影(JDBC/Redis)
           ├─ FileCatalogService → files/file_versions/object store  ← 持久兜底
...
- **企业文件资产**：`files` 管逻辑路径和状态，`file_versions` 管不可变版本，MinIO/S3 管字节；本地测试可退到 PG `file_object_blobs`；
- **多租户路径隔离**：`NamespaceFactory rc → List.of("org", orgId, "user", userId)`（见 [04](./04-iam-and-isolation.md)）；
- **大对象归档**：MinIO 存快照、文件对象、上传、导出。
